...
exports.getCartRecommendations = async (req, res) => {
  console.log(req.body);
	try {
		...
		console.log(
			`🔍 Requesting recommendations for cart ${cart} from ML service...`,
		);

		const {recommendations} = await mlService.getCartRecommendations(cart, {
...
exports.getUserRecommendations = async (req, res) => {
	try {
		...
		console.log(
			`🔍 Requesting recommendations for user ${userId} from ML service...`,
		);
		...
		console.log(
			`✅ Received ${recommendations.restaurants?.length || 0} recommendations`,
		);
...
exports.getSimilarRestaurants = async (req, res) => {
	try {
		...
		const recommendations = await mlService.getSimilarRestaurants(
			restaurantId,
			parseInt(limit),
		);
...
exports.getRecommendationsByCuisine = async (req, res) => {
	try {
		...
		const recommendations = await mlService.getRecommendationsByCuisine(
			cuisine,
			{
				limit: parseInt(limit),
...
exports.getTrendingRestaurants = async (req, res) => {
	try {
		...
		const recommendations = await mlService.getTrendingRestaurants(
			parseInt(limit),
			location,
		);
...
exports.getGroupRecommendations = async (req, res) => {
	try {
		...
		console.log(
			`👥 Finding common restaurant recommendations for ${userIds.length} users...`,
		);
